...
const CreateAdminModal = ({ className, state, data, close }: Props) => {
  const modal = useRef(null);
  const [buttonData, setButtonData] = useState(data);
  const [newData, setNewData] = useState({ name: "", prompt: "" });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    switch (e.target.name) {
      ...
      case "prompt":
        setNewData({ ...newData, prompt: e.target.value });
        break;
